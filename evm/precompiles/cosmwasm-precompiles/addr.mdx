...
```bash # Intitate a hardhat project
npx hardhat init
...
```typescript // Import Address precompile address and ABI
...
```typescript // Associate addresses using a custom message signature
...
```solidity contract AddressManager {
    IAddrPrecompile constant ADDR = IAddrPrecompile(0x0000000000000000000000000000000000001004);
...
```typescript // Associate addresses using a public key
try {
...
```typescript // Query the Sei address for a given EVM address
...
```typescript // Query the EVM address for a given Sei address
...
```typescript import { ADDRESS_PRECOMPILE_ABI, ADDRESS_PRECOMPILE_ADDRESS } from '@sei-js/precompiles';
import { ethers } from 'ethers';
