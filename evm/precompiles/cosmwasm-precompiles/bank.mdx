...
```bash # Instantiate a new hardhat project
npx hardhat init
...
```plaintext RPC_URL=https://evm-rpc.sei-apis.com
...
```typescript // Import Bank precompile address and ABI
...
<Tab title="Solidity">
```solidity // Initialize the contract instance in your Solidity code
...
```typescript // Send native SEI tokens to another address
...
```solidity contract PaymentProcessor {
    IBankPrecompile constant BANK = IBankPrecompile(0x0000000000000000000000000000000000001001);
...
```typescript // Send custom tokens between addresses
...
```solidity contract TokenTransferManager {
    IBankPrecompile constant BANK = IBankPrecompile(0x0000000000000000000000000000000000001001);
...
```typescript // Query specific token balance
...
```solidity contract BalanceChecker {
    IBankPrecompile constant BANK = IBankPrecompile(0x0000000000000000000000000000000000001001);
...
```typescript // Query all token balances for an account
const accountAddress = '0x1234567890123456789012345678901234567890';
...
```solidity contract PortfolioManager {
    IBankPrecompile constant BANK = IBankPrecompile(0x0000000000000000000000000000000000001001);
...
```typescript // Query comprehensive token metadata
const tokenDenom = 'usei';
...
```solidity contract TokenRegistry {
    IBankPrecompile constant BANK = IBankPrecompile(0x0000000000000000000000000000000000001001);
...
```typescript import { BANK_PRECOMPILE_ABI, BANK_PRECOMPILE_ADDRESS } from '@sei-js/precompiles';
import { ethers } from 'ethers';
...
```bash npx hardhat run scripts/deploy.js --network sei
```
...
```bash node integrationExample.js
```
...
```solidity // Example of secure permission patterns
...
```typescript // Handle common transfer errors
try {
...
```typescript // Safe balance checking with error handling
