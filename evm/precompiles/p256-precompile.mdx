...
```solidity contract SecureEnclaveAuth {
...
```solidity contract HardwareMultiSig {
    IP256Verify constant P256_PRECOMPILE = IP256Verify(0x0000000000000000000000000000000000001011);
...
```solidity function isValidPublicKey(bytes32 x, bytes32 y) internal pure returns (bool) {
...
```solidity function isLowS(bytes32 s) internal pure returns (bool) {
...
```solidity function safeVerifyP256(
    bytes32 messageHash,
    bytes32 r,
    bytes32 s,
    bytes32 publicKeyX,
