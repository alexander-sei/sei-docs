...
```typescript // Listen for delegation events
...
```bash # Install ethers.js for smart contract interactions
npm install ethers
...
```typescript // Import Staking precompile address and ABI
...
```typescript // Using EVM-compatible wallet as the signer and provider
...
```typescript // Best practice for handling user input amounts
...
```typescript // Convert delegate msg.value (wei) to uSEI (6 decimals) for reconciliation
...
```typescript // Helper functions for amount conversion
...
```typescript // Sei validator address you want to delegate to (seivaloper... format)
...
<Tab title="Solidity">
```typescript function delegateToValidator(string memory validatorAddress) external payable {
...
```typescript // Undelegate 1 SEI from a validator - Uses 6 decimals for amount parameter
const validatorAddress = 'seivaloper1xyz...';
...
<Tab title="Solidity">
```typescript     function undelegateFromValidator(
        string memory validatorAddress, 
        uint256 amount
    ) external {
...
```typescript // Redelegate 0.5 SEI from one validator to another - Uses 6 decimals for amount parameter
...
<Tab title="Solidity">
```typescript     function redelegateBetweenValidators(
...
```typescript // Get your EVM address as the delegator
...
<Tab title="Solidity">
```typescript     function getDelegationInfo(
        address delegator,
        string memory validatorAddress
...
```typescript // Ed25519 public key in hex format (64 characters)
...
```solidity function createNewValidator(
    string memory pubKeyHex,
    string memory moniker,
    string memory commissionRate,
    string memory commissionMaxRate,
...
```typescript // Update validator parameters
...
```typescript // Get all bonded validators
...
```typescript const validatorAddress = 'seivaloper1xyz...';
...
```typescript const delegator = await signer.getAddress();
...
```typescript // Get all redelegations for a specific delegator
...
```typescript // Get staking pool totals
...
```typescript // Get validator set at a specific block height
...
```typescript async function rebalanceStaking(
...
```typescript async function stakingExample() {
  // Setup
  const provider = new ethers.BrowserProvider(window.ethereum);
  await provider.send('eth_requestAccounts', []);
  const signer = await provider.getSigner();
...
```javascript const abi = require('./artifacts/contracts/Staking.sol/StakingManager.json');
...
```typescript // Set appropriate gas limits for different operations
const delegateTx = await staking.delegate(validatorAddress, {
  value: amount,
  gasLimit: 300000
});
