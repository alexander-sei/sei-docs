...
```typescript // Import Governance precompile address and ABI
...
```typescript // Using MetaMask as the signer and provider (browser environment)
...
```typescript // Validate vote option
...
```typescript async function submitGovernanceProposal(title: string, description: string, depositAmount: bigint, isExpedited: boolean = false): Promise<bigint> {
  // Validate inputs
...
```typescript // Simple voting with comprehensive validation
...
```typescript // Weighted voting with comprehensive validation
...
```typescript // Add deposit to help proposal reach minimum threshold
...
```bash mkdir sei-governance-mainnet
...
```bash npm install ethers @sei-js/precompiles@^2.1.2 dotenv
```

3. **Create a `.env` file:**

```bash PRIVATE_KEY=your_private_key_here
```
...
```bash node governance-mainnet-demo.js
```
...
```typescript // Check minimum deposit requirement based on proposal type
const MINIMUM_DEPOSIT = ethers.parseUnits('3500', 18); // 3,500 SEI minimum
const EXPEDITED_DEPOSIT = ethers.parseUnits('7000', 18); // 7,000 SEI for expedited
...
```typescript // Check if user has staked SEI for voting power
...
```typescript // Validate vote option with descriptive error
...
```typescript // Comprehensive weighted vote validation with helpful errors
