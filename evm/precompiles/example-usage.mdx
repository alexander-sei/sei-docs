...
```bash npm install ethers @sei-js/evm
```
...
```typescript import { STAKING_PRECOMPILE_ABI, STAKING_PRECOMPILE_ADDRESS } from '@sei-js/evm';
...
```typescript import { GOVERNANCE_PRECOMPILE_ABI, GOVERNANCE_PRECOMPILE_ADDRESS } from '@sei-js/evm';
...
```typescript import { JSON_PRECOMPILE_ABI, JSON_PRECOMPILE_ADDRESS } from '@sei-js/evm';
...
```typescript // Simple operations (queries)
...
```typescript // Batch read operations
...
```javascript // multi-precompile-demo.js
