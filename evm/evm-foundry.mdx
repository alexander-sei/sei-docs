...
```bash forge remappings > remappings.txt
```
...
```bash forge script script/DeployCounter.s.sol --rpc-url $SEI_TESTNET_RPC --private-key $PRIVATE_KEY --broadcast
```

Deploy to Sei mainnet:

```bash forge script script/DeployCounter.s.sol --rpc-url $SEI_MAINNET_RPC --private-key $PRIVATE_KEY --broadcast
```
...
```bash # Deploy Counter to testnet
...
```bash npm install ethers@latest
```
