...
```bash npm install @coinbase/agentkit @coinbase/agentkit-langchain viem
```
...
```typescript import { AgentKit, ViemWalletProvider, walletActionProvider, erc20ActionProvider } from '@coinbase/agentkit';
...
```typescript import { getLangChainTools } from '@coinbase/agentkit-langchain';
...
```bash pip install coinbase-agentkit coinbase-agentkit-langchain
```

### Create the wallet provider

```python from coinbase_agentkit import (
    AgentKit,
    AgentKitConfig,
...
```python from coinbase_agentkit_langchain import get_langchain_tools
...
```bash curl --request POST https://api.privy.io/v1/wallets \
...
```bash curl --request POST https://api.privy.io/v1/wallets/wallet_abc123/rpc \
  -u "<your-app-id>:<your-app-secret>" \
  -H "privy-app-id: <your-app-id>" \
...
```bash npm install @privy-io/server-auth
```

### Create a wallet and send a transaction

```typescript import { PrivyClient } from '@privy-io/server-auth';
...
```bash pip install privy-client
```

### Create a wallet and send a transaction

```python from privy import PrivyAPI
...
```typescript // "Cap sends at 10 SEI and block a specific address."
...
```typescript import { AgentKit, PrivyWalletProvider, walletActionProvider, erc20ActionProvider } from '@coinbase/agentkit';
