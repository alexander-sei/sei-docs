...
```env SIM_API_KEY=your_sim_api_key_here
...
```javascript async function getSeiBalances() {
  try {
    const response = await axios.get(`${SIM_API_BASE}/evm/balances/${process.env.TEST_WALLET_ADDRESS}?chain_ids=1329`, { headers });
...
```javascript async function monitorSeiTransactions(address) {
  try {
...
```javascript async function getTokenInfo(tokenAddress) {
  try {
...
```javascript async function getTokenHolders(tokenAddress, limit = 20) {
  try {
...
```javascript async function getWalletActivity(walletAddress, limit = 10) {
  try {
...
```javascript async function getNFTCollections(walletAddress) {
  try {
...
```javascript class RateLimitedClient {
