...
```bash seid q evm sei-addr [evm address]
```

**Example:**

```bash seid q evm sei-addr 0x1234abcd5678ef...
```
...
```bash seid q evm evm-addr [sei address]
```

**Example:**

```bash seid q evm evm-addr sei1xyz...
```
...
```bash seid q evm erc20 [contract address] [method] [arguments...]
```

**Example:**

```bash seid q evm erc20 0xabc123... balanceOf 0xuser456...
```
...
<Tabs>
  <Tab title="ERC20">
    ...
    ```bash     seid q evm erc20-payload [method] [arguments...]
    ```

    **Transfer tokens:**
    ```bash     seid q evm erc20-payload transfer 0xrecipient... 1000000000000000000
    ```
    ...
    **Approve spender:**
    ```bash     seid q evm erc20-payload approve 0xspender... 2000000000000000000
    ```
    ...
    **Transfer from:**
    ```bash     seid q evm erc20-payload transferFrom 0xfrom... 0xto... 1500000000000000000
    ```
...
Amounts should be specified in the token's smallest unit (e.g., wei for 18-decimal tokens).
  ...
  <Tab title="ERC721">
    ...
    ```bash     seid q evm erc721-payload [method] [arguments...]
    ```

    **Approve token:**
    ```bash     seid q evm erc721-payload approve 0xspender... 123
    ```
    ...
    **Transfer token:**
    ```bash     seid q evm erc721-payload transferFrom 0xfrom... 0xto... 123
    ```
    ...
    **Set approval for all:**
    ```bash     seid q evm erc721-payload setApprovalForAll 0xoperator... true
    ```
  ...
  <Tab title="ERC1155">
    ...
    ```bash     seid q evm erc1155-payload [method] [arguments...]
    ```

    **Safe transfer:**
    ```bash     seid q evm erc1155-payload safeTransferFrom 0xfrom... 0xto... 123 5 0x
    ```
    ...
    **Safe batch transfer:**
    ```bash     seid q evm erc1155-payload safeBatchTransferFrom 0xfrom... 0xto... [123,456] [5,10] 0x
    ```
    ...
    **Set approval for all:**
    ```bash     seid q evm erc1155-payload setApprovalForAll 0xoperator... true
    ```
  ...
  <Tab title="Custom ABI">
    ...
    ```bash     seid q evm payload [abi-filepath] [method] [arguments...]
    ```

    **Example:**
    ```bash     seid q evm payload ./MyContract.json myMethod arg1 arg2
    ```
...
```bash seid q evm pointer [type] [pointee]
```

**Example:**

```bash seid q evm pointer NATIVE usei
```
...
```bash seid q evm pointee [type] [pointer]
```

**Example:**

```bash seid q evm pointee ERC20 0xpointer123...
```
...
```bash seid q evm pointer-version [type]
```

**Example:**

```bash seid q evm pointer-version ERC20
```
...
```bash seid q evm tx [hash] --evm-rpc [EVM RPC endpoint]
```

**Example:**

```bash seid q evm tx 0x1234567890abcdef... --evm-rpc http://127.0.0.1:8545
```
...
```bash # Generate transfer payload
...
```bash # Find EVM pointer for a native token
