...
```bash npm install @sei-js/sei-global-wallet
```

or

```bash yarn add @sei-js/sei-global-wallet
```

Import the package to register the wallet:

```js import '@sei-js/sei-global-wallet/eip6963';
```
...
```bash npm create vite@latest my-sei-dapp -- --template react-ts
cd my-sei-dapp
```

**Install Dependencies:**

```bash npm install @rainbow-me/rainbowkit wagmi viem @tanstack/react-query @sei-js/sei-global-wallet
```

**Update App.tsx:**

```tsx import '@rainbow-me/rainbowkit/styles.css';
...
```bash npm install connectkit wagmi viem @tanstack/react-query @sei-js/sei-global-wallet
```

**Update App.tsx:**

```tsx import '@sei-js/sei-global-wallet/eip6963';
...
```bash npm install @web3-react/core @web3-react/eip1193 @web3-react/types @sei-js/sei-global-wallet
```
...
```bash npm install @dynamic-labs/sdk-react-core \
...
```bash npx create-next-app@latest my-sei-dapp --typescript --tailwind --eslint
cd my-sei-dapp
```
...
```bash mkdir sei-global-wallet-bundle
...
```bash npm install --save-dev esbuild
```
...
```bash npm install @web3auth/modal @web3auth/modal/react \
...
```ts import '@sei-js/sei-global-wallet/eip6963';
...
```tsx import { StrictMode } from 'react';
...
```bash # Ensure the import is at the top of your App.tsx
import '@sei-js/sei-global-wallet/eip6963';
```

**Issue:** TypeScript errors about `window.ethereum`

```typescript // Add this to your App.tsx or create types/global.d.ts
declare global {
  interface Window {
    ethereum?: any;
  }
...
```bash # Get a proper project ID from cloud.walletconnect.com
# Replace 'YOUR_WALLETCONNECT_PROJECT_ID' with your actual ID
```

**Issue:** Network not found in MetaMask

```javascript // Use the exact chainId in hex format
...
```javascript // Check if EIP-6963 events are firing
