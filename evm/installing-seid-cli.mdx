...
To install seid, locate the version you wish to use on the [sei-chain releases page](https://github.com/sei-protocol/sei-chain/releases). Always cross-check the current mainnet and testnet binary versions on the [network status dashboard](https://www.sei.io/status) before upgrading a production node.

Execute the following commands with the version you picked:

```bash git clone https://github.com/sei-protocol/sei-chain
cd sei-chain
...
```bash export GOPATH=[your GOPATH from step 1]
...
```bash seid keys add $NAME --recover
```
...
```bash rm $(go env GOPATH)/bin/seid
```
