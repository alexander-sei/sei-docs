...
```bash seid tx evm associate-address [optional priv key hex] --from=<sender> --evm-rpc=<url>
```
...
```bash seid tx evm associate-address --from=mykey --evm-rpc=http://localhost:8545
```
...
```bash seid tx evm send [to EVM address] [amount in wei] --from=<sender> --gas-fee-cap=<cap> --gas-limit=<limit> --evm-rpc=<url>
```
...
```bash seid tx evm send 0x1234567890abcdef1234567890abcdef12345678 1000000000000000000 --from=mykey
```
...
```bash seid tx evm erc20-send [contract addr] [recipient] [amount] --from=<sender> --gas-fee-cap=<cap> --gas-limit=<limit> --evm-rpc=<url>
```
...
```bash seid tx evm erc20-send 0xTokenContract123... 0xRecipient456... 1000000 --from=mykey
```
...
```bash seid tx evm deploy [path to binary] --from=<sender> --gas-fee-cap=<cap> --gas-limit=<limit> --evm-rpc=<url>
```
...
```bash seid tx evm deploy ./MyContract.bin --from=mykey --gas-limit=8000000
```
...
```bash seid tx evm call-contract [addr] [payload hex] --value=<payment> --from=<sender> --gas-fee-cap=<cap> --gas-limit=<limit> --evm-rpc=<url>
```
...
```bash seid tx evm call-contract 0xContract123... 0xa9059cbb000000... --value=1000000000000000000 --from=mykey
```
...
```bash seid tx evm call-precompile [precompile name] [method] [args...] --value=<payment> --from=<sender> --gas-fee-cap=<cap> --gas-limit=<limit> --evm-rpc=<url>
```
...
```bash seid tx evm call-precompile staking delegate seivaloper1... --value=1000000000000000000 --from=mykey
```
...
```bash seid tx evm send 0x... 1000000000000000000 \
