...
```bash pnpm install permissionless viem dotenv
```
...
```shell PIMLICO_API_KEY='pim_xxxxxxxxxxxxxxxxxxxxxx'
```
...
```typescript import { createSmartAccountClient } from 'permissionless';
...
```typescript import { toSimpleSmartAccount } from 'permissionless/accounts';
...
```typescript const txHash = await smartAccountClient.sendTransaction({
