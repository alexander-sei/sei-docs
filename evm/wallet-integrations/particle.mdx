...
keywords: ['particle network', 'social login', 'wallet abstraction', 'account abstraction', 'smart accounts', 'sei']
mode: "wide"
---
...
```bash yarn add @particle-network/connectkit viem@^2 @particle-network/aa ethers
```
...
2. Click **Web** under **Your Apps** (if you intend to use an alternative
   ...
   [platform-specific guides on Particle's documentation](https://developers.particle.network/social-logins/connect/introduction))
...
```shell NEXT_PUBLIC_PROJECT_ID='PROJECT_ID'
...
```tsx import { ParticleConnectkit } from '@/components/Connectkit';
...
```tsx import { useSmartAccount } from '@particle-network/connectkit';
