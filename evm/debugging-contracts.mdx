...
```bash seid tx <module> <action> <parameters> --from <key> --evm-rpc http://url-to-sei-evm-rpc --generate-only
```
...
```bash seid tx evm register-evm-pointer NATIVE \
...
```bash cast tx 0x5010e6600e67f04a9bc3d3b670a7c2de380b180713d9a014a5dbd76b7e2190f1 \
--rpc-url=http://url-to-sei-evm-rpc
```

**Example Output:**

```text blockHash               0x4696d63a9a9ae88b03bcc94ccbd87f407e994b309d1dff9c0626de51ac57b76e
...
```bash cast receipt 0x5010e6600e67f04a9bc3d3b670a7c2de380b180713d9a014a5dbd76b7e2190f1 \
--rpc-url=http://url-to-sei-evm-rpc
```

**Decode transaction input data:**

```bash cast 4byte-decode 0xc31d960f0000...
```
...
```bash # Fetch receipt JSON and view logs
...
```bash cast nonce 0xYourAddress --rpc-url=$EVM_RPC_URL
...
```bash # 1) Generate a template (no broadcast)
...
```bash cast storage 0xContract 0xSLOT --rpc-url=$EVM_RPC_URL
```
...
```bash # Use Foundry's cast tool for detailed transaction inspection
...
```bash # Create .env file for sensitive data
WALLET_NAME=mykey
...
```bash # Use environment variables in commands
