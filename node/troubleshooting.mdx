...
```text Error: "Dial tcp connection refused"
...
```text Error: "Database is corrupted"
...
```bash # Check node synchronization
...
```bash # For SeiDB (most non-archive nodes):
...
```text ERR wrong Block.Header.AppHash. Expected [EXPECTED_HASH], got [ACTUAL_HASH]
...
```text ERR failed to handshake with peer
...
2. **Look for the actual error pattern:**
   ```text    ERR wrong Block.Header.AppHash. Expected [HASH], got [HASH]
   ```
...
```text ERR failed to handshake with peer err="expected to connect with peer \"[EXPECTED_ID]\", got \"[ACTUAL_ID]\""
...
1. **Update peer configurations** with current node IDs from the official peer lists:
   - Mainnet (Pacific-1): [peers.txt](https://github.com/sei-protocol/testnet/blob/main/pacific-1/peers.txt)
   - Testnet (Atlantic-2): [peers.txt](https://github.com/sei-protocol/testnet/blob/main/atlantic-2/peers.txt)

2. **Verify network connectivity:**

   ```bash    # Test connection to peer endpoints
...
3. **Check current peer status:**
   ```bash    curl http://localhost:26657/net_info | jq '.result.peers | length'
...
```bash # Check sync status and lag
curl http://localhost:26657/lag_status | jq .
...
```bash # Example logrotate configuration
...
```bash # Set unlimited core dump size
ulimit -c unlimited
...
3. **Database Issues**
   ...
   - Run database integrity checks using:
     ...
     ```bash      seid debug dump-db | grep -i error
     ```
   ...
   - Consider pruning excessive historical data by adjusting `ss-keep-recent` in
     ...
     ```bash      seid unsafe-reset-all --home=$HOME/.sei --keep-addr-book
     ```

     Alternatively, manually remove old state snapshots to free up space:

     ```bash      rm -rf $HOME/.sei/data/snapshots/*
     ```
