...
```bash seid init <moniker> --chain-id <chain-id> --mode validator
```
...
```bash # Validator consensus key - Used for signing blocks
...
```bash seid tx staking create-validator \
    --amount=1000000usei \
...
```bash # Validator node config.toml
[p2p]
pex = false
...
```bash # 1. Set up new machine with Sei node
