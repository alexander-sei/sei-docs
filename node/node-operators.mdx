...
```bash sudo apt install make gcc git jq chrony curl lz4 wget tar build-essential -y
```
...
```bash # Check for existing Go installation
go version
...
```bash # Clone repository and install
...
```bash export GOPATH=$(go env GOPATH)
...
```bash # Initialize node (default mode is full: RPC/P2P bind to all interfaces)
...
```bash # Configure peers in config.toml
...
```bash # Set Concurrent Workers to 500
...
```bash sed -i 's/^ss-keep-recent = [0-9]*/ss-keep-recent = 0/' ~/.sei/config/app.toml
```
</Info>

<details className="group border border-neutral-200 dark:border-neutral-800">
  <summary className="flex items-center justify-between cursor-pointer p-4 bg-neutral-50 dark:bg-neutral-900/50 rounded-t-lg">
    <span className="text-base font-medium text-neutral-800 dark:text-neutral-200">Advanced Configuration</span>
  </summary>
  <div className="p-5 pt-4 border-t border-neutral-200 dark:border-neutral-800">
...
```toml # Mempool configuration overrides (values that differ from defaults)
[mempool]
...
```bash sudo tee /etc/logrotate.d/sei > /dev/null << EOF
/var/log/sei/*.log {
    daily
    rotate 14
    compress
...
```bash vm.swappiness = 1  # Reduce swapping to disk, ensuring RAM is used efficiently before relying on slower swap space
...
```bash net.core.somaxconn = 32768  # max connections in queued
...
```bash echo "none" > /sys/block/nvme0n1/queue/scheduler  # disable I/O scheduler to reduce latency
